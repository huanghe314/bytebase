...
func FindAndApplyApprovalTemplate(ctx context.Context, stores *store.Store, b *bus.Bus, webhookManager *webhook.Manager, issue *store.IssueMessage) error {
	...
	err = findApprovalTemplateForIssue(ctx, stores, b, webhookManager, issue, approvalSetting)
	if err != nil {
...
func (r *Runner) processIssue(ctx context.Context, ref bus.IssueRef) {
	...
	if err := findApprovalTemplateForIssue(ctx, r.store, r.bus, r.webhookManager, issue, approvalSetting); err != nil {
...
func findApprovalTemplateForIssue(ctx context.Context, stores *store.Store, b *bus.Bus, webhookManager *webhook.Manager, issue *store.IssueMessage, approvalSetting *storepb.WorkspaceApprovalSetting) error {
